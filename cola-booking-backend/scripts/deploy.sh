...
tar cf - .env dist node_modules package.json package-lock.json | ssh ec2-user@$SERVER -i $PRIVATE_KEY_PEM_FILE "tar x -C ${TEMP_FOLDER}"

echo "Deploy [3/3] - Replacing previous running version of ${PROCESS_NAME} by the new one on remote server"
ssh ec2-user@$SERVER -i $PRIVATE_KEY_PEM_FILE "pm2 stop $PROCESS_NAME && rm -rf $DEPLOY_PATH && mv ${TEMP_FOLDER} $DEPLOY_PATH && cd $DEPLOY_PATH && npm install bcrypt && pm2 start $PROCESS_NAME && cd ~ && rm -rf ${TEMP_FOLDER}"
